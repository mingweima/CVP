...
class Primitive(Dataset):
    ...
    def get_segments(self, ids, colors, pos):
        ...
        for i in range(len(colors)):
            ...
            bbox = np.array([pos[0], pos[1], 2*radius, 2*radius])
    ...
    def __getitem__(self, index): #build_one_dataset
        ...
        for i in l:
            ...
            pos = self.get_projected_positions(proj_matrix = proj_matrix, cam_matrix = cam_matrix, pos = poss) #(O, 2)
    ...
    def _build_graph(self, num_obj):
        all_trip = np.zeros([0, num_obj], dtype=np.float32)
